...
const MeasurementPreview = () => {
  ...
  const { measurement, deleteMeasurement, fetchMeasurement, loading, error } =
    useMeasurementStore();
  ...
  }, [user, fetchMeasurement, id]);

  if (loading) return <Spin />;
  if (error) return <Error error={error} />;
  if (!measurement) return <p>No document found.</p>;
  ...
  const handleDeleteMeasurement = async (mid) => {
    ...
    const { success, message } = await deleteMeasurement(mid);
    if (!success) {
      toast({
        title: "Error",
        description: message,
  ...
  return (
    <Box mt={10}>
      ...
      <Alert
        isOpen={isOpen}
        onClose={onClose}
        onConfirm={() => handleDeleteMeasurement(measurement._id)}
